...
namespace :import do
  ...
  task :users, %i(filename team_id clear_existing_runs) => :environment do |_t, args|
    puts '----- args -----'
    puts args.inpect
    ...
    CSV.foreach(args[:filename], headers: true, header_converters: :symbol) do |row|
      if !row[:first_name].nil? && !row[:last_name].nil?
        user = User.where(first_name: row[:first_name], last_name: row[:last_name])[0]
        if !user.nil?
          
          user.runs.destroy_all if args[:clear_existing_runs] == "true"
          
          existing_data = ImportedUserDatum.where(user_id: user.id, team_id: args[:team_id])
          existing_data.destroy_all unless existing_data.nil?
...
def create_base_data(row, team_id, user_id)
  base_data = {}
  base_data[:num_team_runs] = if row.key?(:num_team_runs)
                                row[:num_team_runs]
                              else
                                0
                              end
